  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (client?.id) {
        ...
        onSaved({ ...client, ...clientData, updatedAt: new Date() } as any);
      } else {
  ...
  return (
    ...
    >
      <motion.div
        ...
        animate={{ scale: 1, y: 0 }}
        className="bg-white w-full max-w-md sm:rounded-3xl shadow-2xl overflow-hidden min-h-[100dvh] sm:min-h-0"
      >
        <form onSubmit={handleSubmit} className="flex flex-col h-full sm:h-auto max-h-[100dvh] sm:max-h-[85vh]">
          ...
          <div className="p-6 sm:p-8 space-y-5 sm:space-y-6 overflow-y-auto flex-1 scrollbar-hide">
          ...
          <div className="p-4 sm:p-8 bg-white border-t border-brand-border flex-shrink-0">
            <button
              disabled={loading}
              type="submit"
