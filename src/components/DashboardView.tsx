...
import { Activity, Users, Calendar, TrendingUp, Clock, ChevronRight, ChevronDown, Plus, History, Stethoscope, Trash2, Tag, MessageSquare, CheckCircle2, IndianRupee, Wallet, Phone, Zap, Download, Bell, PieChart as PieChartIcon, BarChart3 } from 'lucide-react';
...
export default function DashboardView({ 
  ...
  const COLORS = ['#3b82f6', '#ec4899', '#f59e0b', '#10b981', '#6366f1', '#a855f7'];

  const getAnalytics = (data: any[]) => {
    let serviceRev = 0;
    let productRev = 0;
    ...
    data.forEach(t => {
      ...
      if (t.services && Array.isArray(t.services)) {
        t.services.forEach((s: any) => {
          const discount = s.discountType === 'percentage' ? ((s.mrp || 0) * (s.discount || 0) / 100) : (s.discount || 0);
          serviceRev += (s.mrp || 0) - discount;
        });
      }
      ...
      if (t.products && Array.isArray(t.products)) {
        t.products.forEach((p: any) => {
          ...
          const discount = p.discountType === 'percentage' ? (basePrice * (p.discount || 0) / 100) : (p.discount || 0);
          productRev += basePrice - discount;
        });
      }

      // Payment Modes
      if (t.splitPayments && Array.isArray(t.splitPayments)) {
        t.splitPayments.forEach((pm: any) => {
          paymentModes[pm.method] = (paymentModes[pm.method] || 0) + (pm.amount || 0);
        });
      } else if (t.paymentMethod && t.paidAmount) {
        // Fallback for older data structure
        paymentModes[t.paymentMethod] = (paymentModes[t.paymentMethod] || 0) + t.paidAmount;
      }
    });

    const paymentData = Object.keys(paymentModes).map(key => ({
      name: key,
  ...
  const tillDateAnalytics = useMemo(() => getAnalytics(allTreatments), [allTreatments]);
  const todayAnalytics = useMemo(() => getAnalytics(treatmentsToday), [treatmentsToday]);
      ...
      {userRole === 'admin' ? (
        <div className="space-y-8">
                          ...
                          <div className="flex items-center gap-3 overflow-hidden">
                            ...
                            <div className="truncate">
                              ...
                              <div className="text-[10px] text-brand-muted font-medium mt-0.5">
                                Added by Staff • {safeFormat(p.date, 'MMM d, h:mm a')}
                              </div>
                            </div>
                          </div>
                          <button 
                        ...
                        <div key={t.id} className="flex items-center justify-between bg-slate-50 p-3 rounded-2xl border border-brand-border hover:border-brand-primary/20 transition-all cursor-pointer" onClick={() => t.parentId && onSelectPatient(t.parentId)}>
                          <div className="flex items-center gap-3 overflow-hidden">
                            ...
                            <div className="truncate">
                              ...
                              <div className="text-[10px] text-brand-muted font-medium mt-0.5">
                                {t.treatmentName} • {safeFormat(t.date, 'h:mm a')}
                              </div>
                            </div>
                          </div>
                            ...
                            <div key={lead.id} className="bg-white p-3 rounded-xl border border-brand-border flex items-center justify-between shadow-sm hover:border-emerald-200 transition-colors">
                               <div className="min-w-0">
                                 ...
                                 <div className="text-[9px] text-brand-muted mt-0.5 flex items-center gap-1 font-medium">
                                    <Clock size={8} /> {safeFormat(lead.appointmentDate, 'dd MMM (EEE)')}
                                 </div>
                               </div>
                               <div className="flex items-center gap-2">
                              ...
                              <div key={item.id} className="bg-white p-3 rounded-xl border border-brand-border flex items-center justify-between shadow-sm">
                                 <div className="min-w-0">
                                   ...
                                   <div className="text-[9px] text-brand-muted mt-0.5 flex items-center gap-1">
                                      <Clock size={8} /> {safeFormat(item.followUpDate, 'MMM d')}
                                   </div>
                                 </div>
          ...
          <div className="flex items-center justify-between px-1">
            ...
            <span className="text-[10px] font-bold text-brand-muted uppercase tracking-widest">Real-time Data</span>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {/* Till Date Summary */}
            <div className="bg-white p-6 rounded-3xl border border-brand-border shadow-sm space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="text-sm font-bold text-brand-secondary">Till Date Summary</h4>
                  <p className="text-[10px] text-brand-muted font-bold uppercase tracking-widest mt-1">Full clinical history aggregated</p>
                </div>
                <div className="px-3 py-1 bg-blue-50 text-brand-primary rounded-full text-[10px] font-bold">Lifetime</div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="h-[250px] w-full">
                  <div className="text-[9px] font-black text-brand-muted uppercase tracking-widest mb-4 text-center">Service vs Product Revenue</div>
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        data={tillDateAnalytics.revenueData}
                        cx="50%"
                        cy="50%"
                        innerRadius={60}
                        outerRadius={80}
                        ...
                        dataKey="value"
                      >
                        {tillDateAnalytics.revenueData.map((entry, index) => (
                ...
                <div className="h-[250px] w-full">
                  <div className="text-[9px] font-black text-brand-muted uppercase tracking-widest mb-4 text-center">Payment Methods Distribution</div>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={tillDateAnalytics.paymentData} layout="vertical">
                      ...
                      <Bar dataKey="value" radius={[0, 4, 4, 0]}>
                        {tillDateAnalytics.paymentData.map((entry, index) => (
            ...
            <div className="bg-white p-6 rounded-3xl border border-brand-border shadow-sm space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="text-sm font-bold text-brand-secondary">Today's Performance</h4>
                  <p className="text-[10px] text-brand-muted font-bold uppercase tracking-widest mt-1">Activity for {safeFormat(new Date(), 'MMM d, yyyy')}</p>
                </div>
                <div className="px-3 py-1 bg-emerald-50 text-emerald-600 rounded-full text-[10px] font-bold">Daily</div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <div className="h-[250px] w-full">
                  <div className="text-[9px] font-black text-brand-muted uppercase tracking-widest mb-4 text-center">Revenue Breakdown</div>
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        data={todayAnalytics.revenueData}
                        cx="50%"
                        cy="50%"
                        innerRadius={60}
                        outerRadius={80}
                        ...
                        dataKey="value"
                      >
                        {todayAnalytics.revenueData.map((entry, index) => (
                          <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                        ))}
                      </Pie>
                      <Tooltip 
                ...
                <div className="h-[250px] w-full">
                  <div className="text-[9px] font-black text-brand-muted uppercase tracking-widest mb-4 text-center">Collection by Mode</div>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={todayAnalytics.paymentData} layout="vertical">
                      <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#f1f5f9" />
                      <XAxis type="number" hide />
                      <YAxis 
                        dataKey="name" 
                      ...
                      />
                      <Bar dataKey="value" radius={[0, 4, 4, 0]}>
                        {todayAnalytics.paymentData.map((entry, index) => (
                      ...
                      <div className="flex items-center gap-3 sm:gap-4 overflow-hidden">
                        ...
                        <div className="truncate">
                          ...
                          <div className="text-[10px] sm:text-[11px] text-brand-muted mt-0.5 flex items-center gap-2">
                            {safeFormat(treatment.date, 'MMM d, h:mm a')}
                              ...
                              <div className="min-w-0 pr-2">
                                ...
                                <div className="flex items-center gap-2 mt-2">
                                  <div className="text-[9px] font-bold text-brand-primary flex items-center gap-1 bg-blue-50/50 px-1.5 py-0.5 rounded">
                                    <Calendar size={10} />
                                    {safeFormat(lead.appointmentDate, 'MMM d, h:mm a')}
                                  </div>
                            ...
                            >
                              <div className="min-w-0 pr-2">
                                ...
                                <div className="flex items-center gap-2 mt-2">
                                  <div className="text-[9px] font-bold text-brand-primary flex items-center gap-1 bg-blue-50/50 px-1.5 py-0.5 rounded">
                                    <Calendar size={10} />
                                    {safeFormat(item.followUpDate, 'MMMM d')}
                                  </div>
                                  <div className="text-[9px] font-bold text-brand-muted flex items-center gap-1">
                                    <Tag size={10} className="opacity-50" />
